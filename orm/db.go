...
func (d *dbBase) InsertValue(q dbQuerier, mi *modelInfo, isMulti bool, names []string, values []interface{}) (int64, error) {
	...
	if isMulti {
		...
		qmarks = fmt.Sprintf("select %s from dual ", qmarks)
		qmarks = strings.Repeat(qmarks+"union ", multi-1) + qmarks

		query = fmt.Sprintf("INSERT INTO %s%s%s (%s%s%s) %s", Q, mi.table, Q, Q, columns, Q, qmarks)
		//query += "select :0,:1,:2 from dual "
		//query += "union "
		//query += "select :0,:1,:2 from dual "
		/*
			insert into product (id,names, price, code)
...
func (d *dbBase) Update(q dbQuerier, mi *modelInfo, ind reflect.Value, tz *time.Location, cols []string) (int64, error) {
	...
	setMiddles := make([]string, len(setNames))
	for i, v := range setNames {
		setMiddles[i] = fmt.Sprintf("%s%s%s = :%s", Q, v, Q, v)
	}
	setColumns := strings.Join(setMiddles, " , ")

	pkName = fmt.Sprintf("%s%s%s = :%s", Q, pkName, Q, pkName)

	query := fmt.Sprintf("UPDATE %s%s%s SET %s WHERE %s", Q, mi.table, Q, setColumns, pkName)
...
func (d *dbBase) Delete(q dbQuerier, mi *modelInfo, ind reflect.Value, tz *time.Location, cols []string) (int64, error) {
	...
	query := fmt.Sprintf("DELETE FROM %s%s%s WHERE %s", Q, mi.table, Q, wheres)
...
func (d *dbBase) UpdateBatch(q dbQuerier, qs *querySet, mi *modelInfo, cond *Condition, params Params, tz *time.Location) (int64, error) {
	...
	for i, v := range columns {
		...
		if c, ok := values[i].(colValue); ok {
			...
			cols = append(cols, col+fmt.Sprintf(" = :%s", v))
		}
	}
